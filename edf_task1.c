...
int main() {
    ...
    //convert exec time and period to float: 
    cpuUtil = (float)execTimeA/periodA + (float)execTimeB/periodB; 
    //print out the CPU utilization: 
    ...
    for (T = 0; T <= 200; T++) {
        ...
        //if A missed its deadline 
        if(T>=absDeadlineA && tA < execTimeA){ 
            printf("process A missed the deadline! not schedulable\n"); }
            
        //if B missed its deadline 
        if(T>=absDeadlineB && tB < execTimeB){ 
            printf("process B missed the deadline! not schedulable\n"); }
        // End of your code
        ...
        if (T % periodA == 0 && T % periodB != 0) {
            ...
            if (tB < execTimeB) { // process B is unfinished yet
                ...
                if(absDeadlineA <= absDeadlineB){
                    ...
                    doA = 1; 
                    printf("when T=%d, program switched to run process A%d!\n", T, jA); }
                //otherwise, continue running Task B 
                // End of your code
